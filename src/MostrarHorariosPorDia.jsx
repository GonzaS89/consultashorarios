...
export default function MostrarHorariosPorDia() {
  ...
  // Forzado de modo claro: eliminamos rastro de clases oscuras
  useEffect(() => {
  ...
  return (
    <div className="bg-[#f8fafc] text-slate-900 min-h-screen w-screen transition-colors duration-300 font-sans">
      
      {/* Header Estilo Mobile Premium */}
      <header className="sticky backdrop-blur-md top-0 z-40 bg-white/70 border-b border-slate-200 px-6 py-5">
        <div className="max-w-xl mx-auto flex justify-between items-center">
          
          <div className="flex items-center gap-3">
            <div className="relative flex items-center justify-center">
              <IoSparkles className="text-emerald-500 text-lg z-10" />
              <motion.div
                initial={{ x: "-150%" }}
                animate={{ x: "150%" }}
        ...
        {/* Tabs Segmentadas */}
        <div className="max-w-xl mx-auto mt-5 p-1 bg-slate-200/50 rounded-2xl flex border border-slate-200">
      ...
      <AnimatePresence>
        {referencia && (
          <motion.div
            ...
            className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-sm flex items-end"
          >
            <motion.div
              initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
              transition={{ type: "spring", damping: 30, stiffness: 300 }}
              className="w-full h-[100dvh] bg-slate-50 lg:rounded-t-[3rem] flex flex-col shadow-2xl overflow-hidden border-t border-slate-200"
            >
              {/* Handle superior */}
              <div className="w-16 h-1.5 bg-slate-300 rounded-full mx-auto mt-4 mb-2 cursor-pointer" onClick={() => setReferencia(null)} />
              
              <div className="px-8 py-4 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-bold truncate tracking-tight uppercase leading-tight max-w-[250px] italic">{referencia.split(" ").slice(1).join(" ")}</h2>
                </div>
                <button onClick={() => setReferencia(null)} className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center text-slate-900 transition-transform active:scale-90">
              ...
              <div className="px-8 mb-6">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-12 py-4 bg-white border border-slate-200 rounded-[1.5rem] focus:ring-2 focus:ring-emerald-500 outline-none font-semibold text-sm shadow-sm transition-all"
                  />
                  {busquedaModal && (
                    <button 
                      onClick={() => setBusquedaModal("")}
                      className="absolute right-4 top-1/2 -translate-y-1/2 w-7 h-7 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 active:scale-90"
                    >
                      <FaTimes size={12} />
                    </button>
                  )}
              ...
              <div className="flex-1 overflow-y-auto px-8 pb-16 space-y-6">
                {horariosFiltrados.length > 0 ? (
                      ...
                      >
                        <div className="flex justify-between items-start mb-6">
                          <div className="flex items-baseline gap-1">
                            <span className={`text-5xl font-black italic tracking-tighter ${esProximo ? "text-emerald-500" : "text-slate-900"}`}>
                              {item.nombre}
                            </span>
                          ...
                          {esProximo && (
                            <motion.div 
                              ...
                              className="flex items-center gap-2 px-4 py-2 bg-emerald-500 rounded-full shadow-lg shadow-emerald-500/30"
                            >
                              ...
                              <div key={i} className="flex gap-4">
                                <div className="flex flex-col items-center">
                                  <div className={`w-3 h-3 rounded-full transition-all duration-500 ${
                                    match ? "bg-emerald-500 ring-4 ring-emerald-500/20" : 
                                    esPrimero || esUltimo ? "bg-slate-900" : "bg-slate-300"
                                  }`} />
                                  {!esUltimo && (
                                    <div className={`w-[1.5px] h-10 ${match ? "bg-emerald-500/30" : "bg-slate-100"}`} />
                                  )}
                                </div>
                  ...
                  <div className="text-center py-24 opacity-20">
                    ...
                    <p className="font-black uppercase tracking-[0.2em] text-sm text-slate-900">Sin servicios disponibles</p>
                  </div>
                )}
              </div>
            </motion.div>
