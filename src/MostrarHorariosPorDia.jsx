...
export default function MostrarHorariosPorDia() {
    ...
    return (
      <div
        ...
        className={`relative p-5 rounded-[2.5rem] border-2 transition-all duration-300 ${
          esProximo 
            ? "bg-white dark:bg-slate-900 border-emerald-500 shadow-[0_20px_50px_rgba(16,_185,_129,_0.15)] scale-[1.02] z-10" 
            : "bg-white dark:bg-slate-900/50 border-slate-100 dark:border-slate-800 shadow-sm"
        }`}
      >
        {/* Header: Hora y Tag de Siguiente */}
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-baseline gap-1">
            <span className={`text-5xl font-black italic tracking-tighter ${
              esProximo ? "text-emerald-500" : "text-slate-900 dark:text-white"
            }`}>
              {item.nombre}
            </span>
          ...
          {item.recorrido.map((p, i) => {
            ...
            return (
              <div key={i} className="flex gap-4">
                {/* Indicador Visual (Línea y punto) */}
                <div className="flex flex-col items-center">
                  <div className={`w-3 h-3 rounded-full transition-all duration-500 ${
                    match ? "bg-emerald-500 ring-4 ring-emerald-500/20" : 
                    esPrimero || esUltimo ? "bg-slate-900 dark:bg-white" : "bg-slate-300 dark:bg-slate-700"
                  }`} />
                  {!esUltimo && (
                    <div className={`w-[1.5px] h-10 ${
                      match ? "bg-emerald-500/30" : "bg-slate-100 dark:bg-slate-800/50"
                    }`} />
                  )}
                </div>
                ...
                <div className="flex flex-col -mt-1 pb-4">
                  <span className={`text-[13px] uppercase font-black tracking-tight transition-all ${
                    match ? "text-emerald-600 dark:text-emerald-400 scale-105 origin-left" : 
                    esPrimero || esUltimo ? "text-slate-900 dark:text-white" : "text-slate-400 dark:text-slate-500"
                  }`}>
                    {p}
                  </span>
                  ...
                  {(esPrimero || esUltimo) && (
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <span className={`text-[8px] uppercase font-black tracking-[0.1em] ${
                        esProximo ? "text-emerald-600/70" : "text-slate-400"
                      }`}>
        ...
        {/* Decoración tipo App Card (Opcional) */}
        {esProximo && (
          <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
            <IoBusSharp size={80} className="-rotate-12" />
          </div>
        )}
      </div>
    );
  ...
  <div className="text-center py-24 opacity-20">
    ...
    <p className="font-black uppercase tracking-[0.2em] text-sm">Sin servicios</p>
  </div>
)}
              </div>
            </motion.div>
